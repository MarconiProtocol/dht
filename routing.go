...
func (n *nTree) isOK(ih InfoHash) bool {
	...
	if len(r.pendingQueries) > maxNodePendingQueries {
		logger.Infof("DHT: Skipping because there are too many queries pending for this dude.")
		logger.Infof("DHT: This shouldn't happen because we should have stopped trying already. Might be a BUG.")
		return false
	}

	recent := r.wasContactedRecently(ih)
	logger.Debugf("wasContactedRecently for ih=%x in node %x@%v returned %v", ih, r.id, r.address, recent)
